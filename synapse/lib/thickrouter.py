'''
Router process — owns all client telepath connections, classifies
...
class ThickRouter:
    '''Router: owns Daemon, classifies queries, dispatches via socketpair RPC.
    ...
    async def start(self):
        ...
        # The router intercepts all t2:init messages via _onTaskV2Init,
        ...
        logger.info('Router started with %d workers', len(self._worker_fds))
    ...
    async def stop(self):
        '''Stop the router, cancel reader tasks, close fds.'''
        self._running = False
    ...
    def _send(self, fd, msg):
        ...
        while mv:
            try:
                ...
                logger.warning('Router: send failed on fd %d: %s', fd, e)
                return False
            mv = mv[sent:]
        return True
    ...
    async def _onTaskV2Init(self, link: s_link.Link, mesg):
        ...
        try:
            ...
            if not isinstance(e, asyncio.CancelledError):
                logger.exception('Router: error on t2:init: %s',

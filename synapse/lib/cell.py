...
class Cell(s_nexus.Pusher, s_telepath.Aware):
    ...
    async def _initSlabFile(self, path, readonly=None, ephemeral=False):
        if readonly is None:
    ...
    def startmain(cls, argv, outp=None):
        ...
        # Fork router on separate port
        router_listen_fd = fork_info['router_listen_fd']
        arbiter.fork_router(router_listen_fd)

...
class Arbiter:
    ...
    def __init__(self) -> None:
        ...
        self._pending_router_restart: bool = False
        self._loop: Optional[asyncio.AbstractEventLoop] = None
        self._router_pid: Optional[int] = None
    ...
    def fork_workers(
        ...
        # Create dispatch socketpairs (router → worker)
        for i in range(num_workers):
        ...
        # Router → writer dispatch channel
    ...
    def fork_router(self, listen_fd: int) -> int:
        '''
        Fork the router process.

        The router owns all client connections on listen_fd, classifies
        queries, and dispatches reads to workers / writes to the writer via
        dedicated dispatch socketpairs.

        Returns:
            PID of the router process.
        '''
        self._router_listen_fd = listen_fd
        ...
        if pid == 0:
            ...
            try:
                ...
                _router_main(listen_fd, dispatch_fds, writer_dispatch_fd)
            except Exception:
        ...
        logger.info('Forked router (pid %d) on fd %d', pid, listen_fd)
        return pid
    ...
    def get_thick_writer_fd(self) -> Optional[int]:
        '''Return the writer-end fd of the router → writer dispatch channel.'''
    ...
    def _in_child(self, worker_id: int) -> None:
        ...
        # Close router writer channel fds (not needed in worker)
    ...
    def _handle_sigchld(self, signum, frame) -> None:
        ...
        if self._router_pid is not None:
            try:
                pid, status = os.waitpid(self._router_pid, os.WNOHANG)
                if pid != 0:
                    ...
                    if not self._shutdown_flag:
                        self._pending_router_restart = True
    ...
    def process_pending_restarts(self) -> None:
        ...
        if self._pending_router_restart:
            self._pending_router_restart = False
            logger.info('Respawning router...')
            self.fork_router(self._router_listen_fd)
    ...
    def _restart_router(self) -> None:
        '''Kill the old router and fork a new one with current dispatch channels.'''
        if self._router_pid is not None:
            try:
                ...
                pass
            try:
                os.waitpid(self._router_pid, os.WNOHANG)
            except ChildProcessError:
                pass
            self._router_pid = None
            self.fork_router(self._router_listen_fd)
...
def _router_main(listen_fd, worker_dispatch_fds, writer_dispatch_fd):
    '''Entry point for the router child process.'''
    ...
    async def _run():
        ...
        print(f'[router pid={os.getpid()}] Starting...', flush=True)
        ...
        await router.start()
        print(f'[router pid={os.getpid()}] Daemon started', flush=True)

        import socket as _socket
        sock = _socket.socket(fileno=listen_fd)
        sock.setblocking(False)
        await router.listen(f'tcp://0.0.0.0', ssl=None, sock=sock)
        print(f'[router pid={os.getpid()}] Listening', flush=True)
